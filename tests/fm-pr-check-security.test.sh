...
SH
  ...
  # x-watch.check.sh once had privileged watcher dispatch. It is an ordinary
  # unregistered check now, and must be quarantined like any other.
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "watcher failed while rejecting an unregistered check: $(cat "$dir/watch-replaced.err")"
  assert_no_grep 'forged-x-ran' "$dir/watch-replaced.out" \
    "watcher executed an unregistered check with a formerly privileged name"
  [ ! -e "$state/b-custom.check.sh" ] && [ ! -L "$state/b-custom.check.sh" ] \
    || fail "unregistered-check scan left the replaced custom check runnable"
  [ ! -e "$state/x-watch.check.sh" ] && [ ! -L "$state/x-watch.check.sh" ] \
    || fail "unregistered-check scan left the formerly privileged name runnable"
  find "$state/.pr-check-quarantine" -name 'b-custom.check.*' -type f | grep . >/dev/null \
    || fail "unregistered-check scan did not quarantine the replaced custom check"
  find "$state/.pr-check-quarantine" -name 'x-watch.check.*' -type f | grep . >/dev/null \
    || fail "unregistered-check scan did not quarantine the formerly privileged name"
